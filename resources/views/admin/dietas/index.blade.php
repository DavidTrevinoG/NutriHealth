    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <!-- Filtro por rangos de calorías -->
                <form action="{{ route('admin.dietas.index') }}" method="GET" class="mb-4">
                    <div class="flex items-center">
                        <label for="rango_id" class="block text-gray-700 font-bold mr-4">Filtrar por Rango:</label>
                        <select name="rango_id" id="rango_id"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            <option value="">Selecciona un rango</option>
                            @foreach ($rangos as $rango)
                                <option value="{{ $rango->id }}">{{ $rango->inicio }} - {{ $rango->fin }}</option>
                            @endforeach
                        </select>
                        <button type="submit"
                            class="btn-rubi-verde btn-rubi-verde:hover text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ml-4">
                            Filtrar
                        </button>
                    </div>
                </form>
                            ...
                            @foreach ($dietas as $dieta)
                                <tr>
                                    ...
                                    <td class="table-actions text-center">
                                        <button onclick="eliminarDieta({{ $dieta->id_dieta }})"
                                            class="btn btn-sm btn-danger text-white">Eliminar</button>
                                        <a href="{{ route('admin.dietas.show', $dieta->id_dieta) }}"
                                            class="btn btn-sm btn-info text-white">Ver Dieta</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
